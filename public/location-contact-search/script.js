...
(function() {
  ...
  let currentPage = 1;
  const pageSize = 50;
  let currentFile = null; // ✅ ফাইলটি সংরক্ষণের জন্য আলাদা ভেরিয়েবল
  ...
  async function extractFromFile(file) {
    ...
    try {
      const text = await file.text(); // ✅ এখন file সঠিক File অবজেক্ট
  ...
  function handleFile(file) {
    currentFile = file; // ✅ ফাইলটি ভেরিয়েবলে সেভ করা হলো
  ...
  processFileBtn.addEventListener('click', () => {
    if (currentFile) {
      extractFromFile(currentFile); // ✅ ভেরিয়েবল থেকে ফাইল নিয়ে কাজ করা
    } else {
